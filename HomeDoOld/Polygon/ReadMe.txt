...
应用程序向导已为您创建了此 Polygon 项目，作为编写动态链接库 (DLL) 的起点。
...
Polygon.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (Polygon.h)
        GUID 定义                                (Polygon_i.c)
        类型库                                  (Polygon.tlb)
        封送处理代码                                 （Polygon_p.c 和 dlldata.c）

Polygon.h
    此文件包含 Polygon.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 Polygon.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
Polygonps.vcproj
	...
	dlldata.c、Polygon_i.c 和 Polygon_p.c，这些文件是
