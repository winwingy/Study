...
void CTrayIcon::InitTrayIcon(HINSTANCE hInst, HWND hWnd, UINT uCallbackMessage,
    const TCHAR* szIconPath, const TCHAR* szTips)
{
    iconData_.cbSize = sizeof(iconData_);
    iconData_.uFlags = NIF_MESSAGE|NIF_ICON|NIF_TIP;
    iconData_.hIcon = (HICON)::LoadImage(hInst, szIconPath, 
        IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
    iconData_.hWnd = hWnd;
    iconData_.uCallbackMessage = uCallbackMessage;
    _tcscpy_s(iconData_.szTip, _countof(iconData_.szTip), szTips);
}
...
void CTrayIcon::ShowBollon(LPCTSTR infoTitle, LPCTSTR info, UINT infoType)
{
    ...
    _tcscpy_s(iconData_.szInfo, sizeof(iconData_.szInfo), info);
    _tcscpy_s(iconData_.szInfoTitle, _countof(iconData_.szInfoTitle), infoTitle);
