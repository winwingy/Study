...
#include "WinDefine.h"
 ...
 namespace HOTKEY_CONFIG_TEXT
 {
     ...
     const TCHAR* CloseScreen = "CloseScreen";
...
tstring WinDefine::GetValueFromConfig(const tstring& strAppName, const tstring& strKeyName, const tstring& strDefault, const tstring& strFileName)
{
    tstring strConfigPath = GetCongfigPath(strFileName);

    char szVolume[2048] = {0};
    GetPrivateProfileString(strAppName.c_str(), strKeyName.c_str(), strDefault.c_str(), szVolume, sizeof(szVolume)-1, strConfigPath.c_str() );	
    return szVolume;
}

int WinDefine::GetValueFromConfig(const tstring& strAppName, const tstring& strKeyName, int strDefault, const tstring& strFileName)
{
    tstring strConfigPath = GetCongfigPath(strFileName);    
    return GetPrivateProfileInt(strAppName.c_str(), strKeyName.c_str(), strDefault, strConfigPath.c_str());
}
