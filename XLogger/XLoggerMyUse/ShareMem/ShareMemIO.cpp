...
const unsigned int ShareFileCount = 100;
...
bool ShareMem::Init()
{
    ...
    memHandleA_.reset(CreateFileMapping((HANDLE)-1, nullptr, PAGE_READWRITE, 0,
        ShareFileCount * sizeof(TCHAR), L"ShareMemUse"));
    ...
    memHandleB_.reset(CreateFileMapping((HANDLE)-1, nullptr, PAGE_READWRITE, 0,
        ShareFileCount * sizeof(TCHAR), guiStrB));
...
bool ShareMem::Write(const TCHAR* writeText, int writeLen, int* writedLen)//0 1 2 3 4 5 6
{
    ...
    do
    {
        ...
        {
            writeWill = writeLen;
            if ((writeLen + wEnd_) > (ShareFileCount - 1))
            {
                writeWill = (ShareFileCount - 1) - wEnd_;
            }
            if (writeWill <= 0)
            {
                while (1)
